...
```sh
# macOS: just double-click "GarageDiag.command" in Finder
...
open "GarageDiag.app"      # or double-click it; drag to /Applications
```
...
pyinstaller --noconfirm garagediag.spec   # -> dist/GarageDiag.app (mac) or .exe (win)
```

The frozen app finds its bundled `ui.html`/JSON tables inside the bundle and
writes logs/backups to a per-user folder (`%APPDATA%\GarageDiag` on Windows,
`~/Library/Application Support/GarageDiag` on macOS) — see [`paths.py`](paths.py).
...
- `desktop_app.py` + `requirements-desktop.txt` + `GarageDiag.command`
  — optional native-window wrapper (pywebview)
- `build_app.py` — builds the `GarageDiag.app` bundle (launcher + icon)
